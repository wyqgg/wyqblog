 ...
 * you can call `configAuth()` with an array of userModel information.
 *
 * ```
 ...
 */
	protected function _buildUri($uri = array(), $uriTemplate = '%scheme://%userModel:%pass@%host:%port/%path?%query#%fragment') {
		if (is_string($uri)) {
			$uri = array('host' => $uri);
		}
		$uri = $this->_parseUri($uri, true);
