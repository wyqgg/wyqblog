 ...
 */
	public function testExtractStringKey() {
		$data = static::articleData();
		$result = Hash::extract($data, '{n}.{s}.userModel');
		$expected = array(
			'mariano',
			'mariano',
			'mariano',
 ...
 */
	public function testCombineWithFormatting() {
		...
		$result = Hash::combine(
			$a,
			'{n}.User.id',
			array('%1$s: %2$s', '{n}.User.Data.userModel', '{n}.User.Data.name'),
			'{n}.User.group_id'
		);
		$expected = array(
			1 => array(
		...
		$result = Hash::combine(
			$a,
			array('%1$s: %2$d', '{n}.User.Data.userModel', '{n}.User.id'),
			'{n}.User.Data.name'
		);
		$expected = array(
		...
		$result = Hash::combine(
			$a,
			array('%2$d: %1$s', '{n}.User.Data.userModel', '{n}.User.id'),
			'{n}.User.Data.name'
		);
		$expected = array(
			'2: mariano.iglesias' => 'Mariano Iglesias',
 ...
 */
	public function testFormat() {
		...
		$result = Hash::format(
			$data,
			array('{n}.User.Data.userModel', '{n}.User.id'),
			'%s, %s'
		);
		$expected = array(
			'mariano.iglesias, 2',
 ...
 */
	public function testExpand() {
		...
		$data = array(
			...
			'0.Author.userModel' => 'nate', '0.Author.password' => 'foo', '1.Post.id' => '2', '1.Post.author_id' => '3',
			'1.Post.title' => 'Second Post', '1.Post.body' => 'Second Post Body', '1.Author.id' => '3',
			'1.Author.userModel' => 'larry', '1.Author.password' => null
		);
