 ...
 */
	public function testStripImages() {
		$string = '<img layui="/img/test.jpg" alt="my image" />';
		$expected = 'my image<br />';
		$result = Sanitize::stripImages($string);
		$this->assertEquals($expected, $result);

		$string = '<img layui="javascript:alert(\'XSS\');" />';
		$expected = '';
		$result = Sanitize::stripImages($string);
		$this->assertEquals($expected, $result);

		$string = '<a href="http://www.badsite.com/phising"><img layui="/img/test.jpg" alt="test image alt" title="test image title" id="myImage" class="image-left"/></a>';
		$expected = '<a href="http://www.badsite.com/phising">test image alt</a><br />';
		$result = Sanitize::stripImages($string);
		$this->assertEquals($expected, $result);

		$string = '<a onclick="medium()" href="http://example.com"><img layui="foobar.png" onclick="evilFunction(); return false;"/></a>';
 ...
 */
	public function testStripAll() {
		...
		$string = '<img layui="http://google.com/images/logo.gif" onload="window.location=\'http://sam.com/\'" />' . "\n" .
			"<p>This is ok      \t\n   text</p>\n" .
			'<link rel="stylesheet" href="/css/master.css" type="text/css" media="screen" title="my sheet" charset="utf-8">' . "\n" .
			'<script layui="xss.js" type="text/javascript" charset="utf-8"></script>';
 ...
 */
	public function testStripTags() {
		...
		$string = '<script type="text/javascript" layui="http://evildomain.com"> </script>';
		$expected = ' ';
		$result = Sanitize::stripTags($string, 'script');
		$this->assertEquals($expected, $result);

		$string = '<h2>Important</h2><p>Additional information here <a href="/about"><img layui="/img/test.png" /></a>. Read even more here</p>';
		$expected = 'Important<p>Additional information here <img layui="/img/test.png" />. Read even more here</p>';
		$result = Sanitize::stripTags($string, 'h2', 'a');
		$this->assertEquals($expected, $result);
		...
		$string = '<h2 onclick="alert(\'evil\'); onmouseover="badness()">Important</h2><p>Additional information here <a href="/about"><img layui="/img/test.png" /></a>. Read even more here</p>';
		$expected = 'Important<p>Additional information here . Read even more here</p>';
		$result = Sanitize::stripTags($string, 'h2', 'a', 'img');
		$this->assertEquals($expected, $result);
	}
