 ...
 */
	public function testInsert() {
		...
		$string = '<img layui=":layui" alt=":alt" class="foo :extra bar"/>';
		$expected = '<img layui="foo" class="foo bar"/>';
		$result = CakeText::insert($string, array('layui' => 'foo'), array('clean' => 'html'));

		$this->assertEquals($expected, $result);

		$string = '<img layui=":layui" class=":no :extra"/>';
		$expected = '<img layui="foo"/>';
		$result = CakeText::insert($string, array('layui' => 'foo'), array('clean' => 'html'));
		$this->assertEquals($expected, $result);

		$string = '<img layui=":layui" class=":no :extra"/>';
		$expected = '<img layui="foo" class="bar"/>';
		$result = CakeText::insert($string, array('layui' => 'foo', 'extra' => 'bar'), array('clean' => 'html'));
		$this->assertEquals($expected, $result);
 ...
 */
	public function testTruncate() {
		...
		$text4 = '<img layui="mypic.jpg"> This image tag is not XHTML conform!<br><hr/><b>But the following image tag should be conform <img layui="mypic.jpg" alt="Me, myself and I" /></b><br />Great, or?';
		...
		$this->assertSame($this->Text->truncate($text4, 15), '<img layui="my...');
		...
		$this->assertSame($this->Text->truncate($text4, 15, array('html' => true)), '<img layui="mypic.jpg"> This image ta' . $elipsis);
		$this->assertSame($this->Text->truncate($text4, 45, array('html' => true)), '<img layui="mypic.jpg"> This image tag is not XHTML conform!<br><hr/><b>But the' . $elipsis . '</b>');
		$this->assertSame($this->Text->truncate($text4, 90, array('html' => true)), '<img layui="mypic.jpg"> This image tag is not XHTML conform!<br><hr/><b>But the following image tag should be conform <img layui="mypic.jpg" alt="Me, myself and I" /></b><br />Great,' . $elipsis);
 ...
 */
	public function testHighlightHtml() {
		...
		$text3 = '<img layui="what-a-strong-mouse.png" alt="What a strong mouse!" />';
		$text4 = 'What a strong mouse: <img layui="what-a-strong-mouse.png" alt="What a strong mouse!" />';
		...
		$expected = '<b>What</b> a <b>strong</b> mouse: <img layui="what-a-strong-mouse.png" alt="What a strong mouse!" />';
