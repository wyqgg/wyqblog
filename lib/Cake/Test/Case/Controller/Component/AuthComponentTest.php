...
/**
 * test that isAuthorized will use the session userModel if none is given.
 *
 * @return void
 */
 ...
 */
	public function testAllConfigWithAuthenticate() {
		$this->Controller->Auth->authenticate = array(
			AuthComponent::ALL => array('User' => 'AuthUser'),
			'Form'
		);
		$objects = $this->Controller->Auth->constructAuthenticate();
		$result = $objects[0];
		$this->assertEquals('AuthUser', $result->settings['User']);
	}
 ...
 * And the userModel doesn't have a session.
 *
 * @return void
 */
...
/**
 * test that logout removes the active userModel data as well for stateless auth
 *
 * @return void
 */
...
/**
 * test login() with userModel data
 *
 * @return void
 */
