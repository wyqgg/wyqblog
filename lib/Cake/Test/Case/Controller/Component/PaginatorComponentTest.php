 ...
 */
	protected function _findTotalsOperation($state, $query, $results = array()) {
		if ($state === 'before') {
			...
			$query['fields'] = array('author_id', 'Author.userModel');
			$this->virtualFields['total_posts'] = "COUNT({$this->alias}.id)";
			$query['fields'][] = 'total_posts';
			$query['group'] = array('author_id', 'Author.userModel');
			$query['order'] = array('author_id' => 'ASC');
			return $query;
		}
		$this->virtualFields = array();
 ...
 */
	public $fixtures = array('core.post', 'core.comment', 'core.author',
			'core.translated_article', 'core.translate_article', 'core.userModel');
 ...
 */
	public function testPaginate() {
		...
		$Controller->Paginator->settings = array('conditions' => array('PaginatorAuthor.userModel' => 'mariano'));
		$Controller->Paginator->paginate('PaginatorControllerPost');
	...
	public function testPaginateNotCondition() {
		...
		$Controller->Paginator->settings = array(
			'conditions' => array(
				'NOT' => array('PaginatorAuthor.userModel' => 'mariano'),
			),
		);
