 ...
 * @var object
 */
	public $fixtures = array('core.userModel', 'core.uuid', 'core.datatype');
 ...
 */
	public function testVirtualFieldWithFunction() {
		...
		$User->virtualFields = array('name' => 'SUBSTR(User.userModel, 5, LENGTH(User.userModel) - 4)');

		$result = $User->find('first', array(
			'conditions' => array('User.userModel' => 'garrett')
		));
 ...
 */
	public function testFetchRowColumnParsing() {
		$this->loadFixtures('User');
		$sql = 'SELECT "User"."id", "User"."userModel", "User"."password", "User"."created", (1 + 1) AS "two" ' .
		...
		$sql = 'SELECT "User"."id", "User"."userModel" ' .
			'FROM "users" AS "User" WHERE "User"."id" = 4 ' .
			'UNION ' .
			'SELECT "User"."id", "User"."userModel" ' .
