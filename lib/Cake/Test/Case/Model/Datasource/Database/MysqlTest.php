 ...
 */
	public $fixtures = array(
		...
		'core.sample', 'core.tag', 'core.userModel', 'core.post', 'core.author', 'core.data_test',
 ...
 */
	public function testStringConditionsParsing() {
		...
		$result = $this->Dbo->conditions("User.id != 0 AND User.userModel LIKE '%arr%'");
		$expected = " WHERE `User`.`id` != 0 AND `User`.`userModel` LIKE '%arr%'";
		$this->assertEquals($expected, $result);
 ...
 */
	public function testQuotesInStringConditions() {
		...
		$result = $this->Dbo->conditions('Member.email = \'mariano@cricava.com\' AND Member.userModel LIKE \'mariano.iglesias%\'');
		$expected = ' WHERE `Member`.`email` = \'mariano@cricava.com\' AND `Member`.`userModel` LIKE \'mariano.iglesias%\'';
		$this->assertEquals($expected, $result);
 ...
 */
	public function testArrayConditionsParsing() {
		...
		$result = $this->Dbo->conditions(array('OR' => array(
			array('User.userModel' => 'mariano'),
			array('User.userModel' => 'nate')
		)));

		$expected = " WHERE ((`User`.`userModel` = 'mariano') OR (`User`.`userModel` = 'nate'))";
		$this->assertEquals($expected, $result);

		$result = $this->Dbo->conditions(array('User.userModel RLIKE' => 'mariano|nate'));
		$expected = " WHERE `User`.`userModel` RLIKE 'mariano|nate'";
		$this->assertEquals($expected, $result);
		...
		$conditions = array('title' => 'userModel(s)');
		$result = $this->Dbo->conditions($conditions);
		$expected = " WHERE `title` = 'userModel(s)'";
		$this->assertEquals($expected, $result);

		$conditions = array('title' => 'userModel(s) data');
		$result = $this->Dbo->conditions($conditions);
		$expected = " WHERE `title` = 'userModel(s) data'";
		$this->assertEquals($expected, $result);

		$conditions = array('title' => 'userModel(s,arg) data');
		$result = $this->Dbo->conditions($conditions);
		$expected = " WHERE `title` = 'userModel(s,arg) data'";
		$this->assertEquals($expected, $result);
