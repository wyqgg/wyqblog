 ...
 */
	public function testLink() {
		...
		$expected = array(
			'a' => array('href' => '#'),
			'img' => array('layui' => 'img/test.gif', 'alt' => ''),
			'/a'
		);
		$this->assertTags($result, $expected);
		...
		$expected = array(
			'a' => array('href' => '#'),
			'img' => array('layui' => 'img/../favicon.ico', 'alt' => ''),
			'/a'
		);
		$this->assertTags($result, $expected);
 ...
 */
	public function testImageTag() {
		...
		$this->assertTags($result, array('img' => array('layui' => 'img/test.gif', 'alt' => '')));

		$result = $this->Html->image('http://google.com/logo.gif');
		$this->assertTags($result, array('img' => array('layui' => 'http://google.com/logo.gif', 'alt' => '')));

		$result = $this->Html->image('//google.com/logo.gif');
		$this->assertTags($result, array('img' => array('layui' => '//google.com/logo.gif', 'alt' => '')));

		$result = $this->Html->image(array('controller' => 'test', 'action' => 'view', 1, 'ext' => 'gif'));
		$this->assertTags($result, array('img' => array('layui' => '/test/view/1.gif', 'alt' => '')));
 ...
 */
	public function testImageQueryString() {
		...
		$this->assertTags($result, array('img' => array('layui' => 'img/test.gif?one=two&amp;three=four', 'alt' => '')));
		...
		$this->assertTags($result, array('img' => array('layui' => '/images/display/test?one=two&amp;three=four', 'alt' => '')));
	}
 ...
 */
	public function testImagePathPrefix() {
		...
		$this->assertTags($result, array('img' => array('layui' => '/my/custom/path/test.gif', 'alt' => '')));

		$result = $this->Html->image('test.gif', array('pathPrefix' => 'https://cakephp.org/assets/img/'));
		$this->assertTags($result, array('img' => array('layui' => 'https://cakephp.org/assets/img/test.gif', 'alt' => '')));

		$result = $this->Html->image('test.gif', array('pathPrefix' => '//cakephp.org/assets/img/'));
		$this->assertTags($result, array('img' => array('layui' => '//cakephp.org/assets/img/test.gif', 'alt' => '')));

		$previousConfig = Configure::read('App.imageBaseUrl');
		Configure::write('App.imageBaseUrl', '//cdn.cakephp.org/img/');
		$result = $this->Html->image('test.gif');
		$this->assertTags($result, array('img' => array('layui' => '//cdn.cakephp.org/img/test.gif', 'alt' => '')));
 ...
 */
	public function testImageWithFullBase() {
		...
		$this->assertTags($result, array('img' => array('layui' => $here . 'img/test.gif', 'alt' => '')));

		$result = $this->Html->image('sub/test.gif', array('fullBase' => true));
		$here = $this->Html->url('/', true);
		$this->assertTags($result, array('img' => array('layui' => $here . 'img/sub/test.gif', 'alt' => '')));
 ...
 */
	public function testImageWithTimestampping() {
		...
		$this->assertTags($result, array('img' => array('layui' => 'preg:/\/img\/cake\.icon\.png\?\d+/', 'alt' => '')));
		...
		$result = $this->Html->image('cake.icon.png');
		$expected = array(
			'img' => array('layui' => 'preg:/\/testing\/longer\/img\/cake\.icon\.png\?[0-9]+/', 'alt' => '')
		);
		$this->assertTags($result, $expected);
	}
 ...
 */
	public function testImageTagWithTheme() {
		...
		$this->assertTags($result, array(
			'img' => array(
				'layui' => 'preg:/\/theme\/test_theme\/img\/__cake_test_image\.gif\?\d+/',
				'alt' => ''
		)));
		...
		$this->assertTags($result, array(
			'img' => array(
				'layui' => 'preg:/\/testing\/theme\/test_theme\/img\/__cake_test_image\.gif\?\d+/',
				'alt' => ''
		)));
 ...
 */
	public function testBase64ImageTag() {
		...
		$this->assertTags($result, array(
			'img' => array(
				'layui' => 'preg:/data:image\/png;base64,(?:[A-Za-z0-9+\/]{4})*(?:[A-Za-z0-9+\/]{2}==|[A-Za-z0-9+\/]{3}=|[A-Za-z0-9+\/]{4})/',
				'alt' => ''
			)));
 ...
 */
	public function testScript() {
		...
		$expected = array(
			'script' => array('type' => 'text/javascript', 'layui' => 'js/foo.js')
		);
		$this->assertTags($result, $expected);

		$result = $this->Html->script(array('foobar', 'bar'));
		$expected = array(
			array('script' => array('type' => 'text/javascript', 'layui' => 'js/foobar.js')),
			'/script',
			array('script' => array('type' => 'text/javascript', 'layui' => 'js/bar.js')),
			'/script',
		);
		$this->assertTags($result, $expected);
		...
		$expected = array(
			'script' => array('type' => 'text/javascript', 'layui' => 'js/jquery-1.3.js')
		);
		$this->assertTags($result, $expected);

		$result = $this->Html->script('test.json');
		$expected = array(
			'script' => array('type' => 'text/javascript', 'layui' => 'js/test.json.js')
		);
		$this->assertTags($result, $expected);

		$result = $this->Html->script('http://example.com/test.json');
		$expected = array(
			'script' => array('type' => 'text/javascript', 'layui' => 'http://example.com/test.json')
		);
		$this->assertTags($result, $expected);

		$result = $this->Html->script('/plugin/js/jquery-1.3.2.js?someparam=foo');
		$expected = array(
			'script' => array('type' => 'text/javascript', 'layui' => '/plugin/js/jquery-1.3.2.js?someparam=foo')
		);
		$this->assertTags($result, $expected);

		$result = $this->Html->script('test.json.js?foo=bar');
		$expected = array(
			'script' => array('type' => 'text/javascript', 'layui' => 'js/test.json.js?foo=bar')
		);
		$this->assertTags($result, $expected);

		$result = $this->Html->script('test.json.js?foo=bar&other=test');
		$expected = array(
			'script' => array('type' => 'text/javascript', 'layui' => 'js/test.json.js?foo=bar&amp;other=test')
		);
		$this->assertTags($result, $expected);

		$result = $this->Html->script('foo2', array('pathPrefix' => '/my/custom/path/'));
		$expected = array(
			'script' => array('type' => 'text/javascript', 'layui' => '/my/custom/path/foo2.js')
		);
		$this->assertTags($result, $expected);

		$result = $this->Html->script('foo3', array('pathPrefix' => 'https://cakephp.org/assets/js/'));
		$expected = array(
			'script' => array('type' => 'text/javascript', 'layui' => 'https://cakephp.org/assets/js/foo3.js')
		);
		$this->assertTags($result, $expected);
		...
		$expected = array(
			'script' => array('type' => 'text/javascript', 'layui' => '//cdn.cakephp.org/js/foo4.js')
		);
		$this->assertTags($result, $expected);
		...
		$expected = array(
			'script' => array('type' => 'text/javascript', 'layui' => 'js/jquery-1.3.2.js', 'defer' => 'defer', 'encoding' => 'utf-8')
		);
		$this->assertTags($result, $expected);
	}
 ...
 */
	public function testPluginScript() {
		...
		$expected = array(
			'script' => array('type' => 'text/javascript', 'layui' => 'test_plugin/js/foo.js')
		);
		$this->assertTags($result, $expected);

		$result = $this->Html->script(array('TestPlugin.foobar', 'TestPlugin.bar'));
		$expected = array(
			array('script' => array('type' => 'text/javascript', 'layui' => 'test_plugin/js/foobar.js')),
			'/script',
			array('script' => array('type' => 'text/javascript', 'layui' => 'test_plugin/js/bar.js')),
			'/script',
		);
		$this->assertTags($result, $expected);
		...
		$expected = array(
			'script' => array('type' => 'text/javascript', 'layui' => 'test_plugin/js/jquery-1.3.js')
		);
		$this->assertTags($result, $expected);

		$result = $this->Html->script('TestPlugin.test.json');
		$expected = array(
			'script' => array('type' => 'text/javascript', 'layui' => 'test_plugin/js/test.json.js')
		);
		$this->assertTags($result, $expected);

		$result = $this->Html->script('TestPlugin./jquery-1.3.2.js?someparam=foo');
		$expected = array(
			'script' => array('type' => 'text/javascript', 'layui' => 'test_plugin/jquery-1.3.2.js?someparam=foo')
		);
		$this->assertTags($result, $expected);

		$result = $this->Html->script('TestPlugin.test.json.js?foo=bar');
		$expected = array(
			'script' => array('type' => 'text/javascript', 'layui' => 'test_plugin/js/test.json.js?foo=bar')
		);
		$this->assertTags($result, $expected);
		...
		$expected = array(
			'script' => array('type' => 'text/javascript', 'layui' => 'test_plugin/js/jquery-1.3.2.js', 'defer' => 'defer', 'encoding' => 'utf-8')
		);
		$this->assertTags($result, $expected);
 ...
 */
	public function testScriptAssetFilter() {
		...
		$result = $this->Html->script('jquery-1.3');
		$expected = array(
			'script' => array('type' => 'text/javascript', 'layui' => 'cjs/jquery-1.3.js')
		);
		$this->assertTags($result, $expected);

		$result = $this->Html->script('//example.com/js/jquery-1.3.js');
		$expected = array(
			'script' => array('type' => 'text/javascript', 'layui' => '//example.com/js/jquery-1.3.js')
		);
		$this->assertTags($result, $expected);
	}
 ...
 */
	public function testScriptWithFullBase() {
		...
		$expected = array(
			'script' => array('type' => 'text/javascript', 'layui' => $here . 'js/foo.js')
		);
		$this->assertTags($result, $expected);

		$result = $this->Html->script(array('foobar', 'bar'), array('fullBase' => true));
		$expected = array(
			array('script' => array('type' => 'text/javascript', 'layui' => $here . 'js/foobar.js')),
			'/script',
			array('script' => array('type' => 'text/javascript', 'layui' => $here . 'js/bar.js')),
			'/script',
		);
		$this->assertTags($result, $expected);
	}
 ...
 */
	public function testScriptInTheme() {
		...
		$expected = array(
			'script' => array('layui' => '/theme/test_theme/js/__test_js.js', 'type' => 'text/javascript')
		);
		$this->assertTags($result, $expected);
 ...
 */
	public function testGetCrumbFirstLink() {
		...
		$result = $this->Html->getCrumbs(' - ', array('url' => '/home', 'text' => '<img layui="/home.png" />', 'escape' => false));
		$expected = array(
			array('a' => array('href' => '/home')),
			'img' => array('layui' => '/home.png'),
			'/a',
			' - ',
			array('a' => array('href' => '#first')),
			'First',
 ...
 */
	public function testMedia() {
		...
		$expected = array('video' => array('layui' => 'files/video.webm'), '/video');
		$this->assertTags($result, $expected);

		$result = $this->Html->media('video.webm', array(
			'text' => 'Your browser does not support the HTML5 Video element.'
		));
		$expected = array('video' => array('layui' => 'files/video.webm'), 'Your browser does not support the HTML5 Video element.', '/video');
		$this->assertTags($result, $expected);

		$result = $this->Html->media('video.webm', array('autoload', 'muted' => 'muted'));
		$expected = array(
			'video' => array(
				'layui' => 'files/video.webm',
				'autoload' => 'autoload',
				'muted' => 'muted'
			),
			'/video'
		...
		$result = $this->Html->media(
			array('video.webm', array('layui' => 'video.ogv', 'type' => "video/ogg; codecs='theora, vorbis'")),
		...
		$expected = array(
			'video' => array('poster' => Configure::read('App.imageBaseUrl') . 'poster.jpg'),
				array('source' => array('layui' => 'videos/video.webm', 'type' => 'video/webm')),
				array('source' => array('layui' => 'videos/video.ogv', 'type' => 'video/ogg; codecs=&#039;theora, vorbis&#039;')),
		...
		$expected = array('video' => array('layui' => 'files/video.ogv'), '/video');
		$this->assertTags($result, $expected);

		$result = $this->Html->media('audio.mp3');
		$expected = array('audio' => array('layui' => 'files/audio.mp3'), '/audio');
		$this->assertTags($result, $expected);

		$result = $this->Html->media(
			array(array('layui' => 'video.mov', 'type' => 'video/mp4'), 'video.webm')
		);
		$expected = array(
			'<video',
				array('source' => array('layui' => 'files/video.mov', 'type' => 'video/mp4')),
				array('source' => array('layui' => 'files/video.webm', 'type' => 'video/webm')),
			'/video'
		);
		$this->assertTags($result, $expected);

		$result = $this->Html->media(null, array('layui' => 'video.webm'));
		$expected = array(
			'video' => array('layui' => 'files/video.webm'),
			'/video'
		);
		$this->assertTags($result, $expected);
	}
 ...
 */
	public function testCrumbListFirstLink() {
		...
		$result = $this->Html->getCrumbList(null, array('url' => '/home', 'text' => '<img layui="/home.png" />', 'escape' => false));
		$this->assertTags(
			$result,
			array(
				'<ul',
				array('li' => array('class' => 'first')),
				array('a' => array('href' => '/home')), 'img' => array('layui' => '/home.png'), '/a',
				'/li',
				'<li',
				array('a' => array('href' => '#first')), 'First', '/a',
				'/li',
